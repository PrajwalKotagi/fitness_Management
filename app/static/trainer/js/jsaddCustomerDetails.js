    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        for (var i = 0; i < response.length; i++) {
          var j = i + 1;
        let image1 = response[i].tc_image.substring(3);
        let image2 = response[i].tc_proof.substring(3);
        // let image3 = response[i].tc_otherproof.substring(3);


          
          $("#tableData").append('<tr><td>' + j + '</td><td style="display: none;">' + response[i].tc_id +  '</td><td>' + response[i].tc_name +'</td><td>'+ response[i].tc_address + '</td><td>'+ response[i].tc_zipcode + '</td><td>'+ response[i].tc_birth +'</td><td>'+ response[i].tc_contact +'</td><td>'+ response[i].tc_email +'</td><td><a href="'+image1+'" download>download</a></td><td>'+ response[i].tc_height +'</td><td>'+ response[i].tc_weight +'</td><td>'+ response[i].tc_nationality +'</td><td>'+ response[i].tc_previousgym +'</td><td>'+ response[i].tc_yeartraning +'</td><td>'+ response[i].tc_joining +'</td><td>'+ response[i].tc_age +'</td><td>'+ response[i].tc_gender +'</td><td><a href="'+image2+'" download>download</a></td><td><div class="d-flex" style="justify-content: space-evenly;"><a href="javascript:void(0);" id="edit_row" title="View/Edit" data-bs-toggle="modal" data-bs-target="#edit_modal"  class="text-primary" onClick="getRowsUpdate();">Edit</a><a href="javascript:void(0);" title="Delete" data-bs-toggle="modal" data-bs-target="#delete_modal" class="text-danger" id="delete_row" onClick="getRowsDelete();">Delete</a></div></td></tr>');
        }
      },
      error: function (request, error) {
        console.error(error);
